...
export function guideText() {
    return [
        ...
        `  4. ${chalk.green("accounts")}     Check ASP approval status`,
        `  5. ${chalk.green("withdraw")}     Withdraw from a pool (once approved)`,
        `  6. ${chalk.green("balance")}      Check balances`,
        `  *  ${chalk.green("sync")}         Re-sync on-chain state (most commands sync automatically)`,
        `  *  ${chalk.green("status")}       Check setup anytime`,
        `  *  ${chalk.green("exit")}         Public withdrawal without ASP approval (alias: ragequit)`,
        "",
