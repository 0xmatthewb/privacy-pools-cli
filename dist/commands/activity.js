...
export function parsePositiveInt(raw, fieldName) {
    ...
    if (!Number.isInteger(parsed) || parsed <= 0) {
        throw new CLIError(`Invalid --${fieldName} value: ${raw}.`, "INPUT", `--${fieldName} must be a positive integer.`);
    }
    return parsed;
}
/** @internal Exported for unit testing. */
...
function normalizeActivityEvent(event, fallbackSymbol) {
    const pool = event.pool ?? {};
    const chainId = parseNumberish(pool.chainId);
    const amountRaw = typeof event.amount === "string"
        ? event.amount
        : typeof event.publicAmount === "string"
            ? event.publicAmount
            : null;
    const symbol = typeof pool.tokenSymbol === "string" && pool.tokenSymbol.trim() !== ""
        ? pool.tokenSymbol
        : fallbackSymbol ?? null;
    ...
    return new Command("activity")
        ...
        try {
            ...
            if (opts.asset) {
                ...
                const response = await fetchPoolEvents(chainConfig, pool.scope, page, perPage);
                spin.stop();
                ...
                printTable(["Type", "Pool", "Amount", "Status", "Time", "Tx"], events.map((e) => [
                    e.type,
                    eventPoolLabel(e),
                    e.amountFormatted,
                    e.reviewStatus ?? "-",
