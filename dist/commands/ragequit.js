    ...
    return new Command("ragequit")
        ...
        try {
            ...
            if (poolCommitments.length === 0) {
                spin.stop();
                throw new CLIError("No spendable commitments found for ragequit.", "INPUT", "You may not have any deposits in this pool.");
            }
            spin.stop();
            // Select commitment
            let commitment;
            if (opts.commitment !== undefined) {
                ...
                if (isNaN(idx) || idx < 0 || idx >= poolCommitments.length) {
                    throw new CLIError(`Invalid commitment index: ${opts.commitment}. Valid range: 0-${poolCommitments.length - 1}`, "INPUT");
                }
            ...
            if (isDryRun) {
                ...
                if (isJson) {
                    ...
                    info(`Selected commitment: label=${commitment.label.toString()} value=${formatAmount(commitment.value, pool.decimals, pool.symbol)}`, silent);
            ...
            if (isJson) {
                ...
                process.stderr.write("\n");
                success(`Recovered ${formatAmount(commitment.value, pool.decimals, pool.symbol)}`, silent);
