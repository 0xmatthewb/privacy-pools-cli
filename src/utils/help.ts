...
export function guideText(): string {
  return [
    ...
    chalk.dim("  Read-only dashboards like pools/activity/stats/accounts default to all CLI-supported mainnet chains unless you pass --chain."),
    "",
    ...
    `  3. ${highlight("flow watch")}     Resume a saved workflow through funding, approval, delay, and withdrawal`,
    `  4. ${highlight("flow ragequit")}  Saved-workflow public recovery if you stop waiting or the easy path is declined`,
    ...
    `  8. ${highlight("migrate status")} Read-only legacy check on CLI-supported chains`,
