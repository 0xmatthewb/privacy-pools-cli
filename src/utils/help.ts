...
export function welcomeScreen(): string {
  const lines = [
    ...
    `    ${highlight("init")}  ${highlight("flow")}  ${highlight("deposit")}  ${highlight("withdraw")}  ${highlight("ragequit")}  ${highlight("accounts")}  ${highlight("history")}  ${highlight("sync")}`,
    "",
...
export function guideText(): string {
  return [
    ...
    chalk.dim("  Saved easy-path workflows use 'flow ragequit' as their canonical public recovery path."),
    "",
    ...
    `  Exception: ${highlight("flow start --new-wallet")} creates and uses a dedicated per-workflow wallet instead of the configured signer.`,
    "",
    chalk.bold("Workflow"),
    `  1. ${highlight("init")}           Set up wallet and config (run once)`,
    `  2. ${highlight("flow start")}     Easy path: deposit now and save a later private withdrawal`,
    `  3. ${highlight("flow watch")}     Resume a saved workflow until approval and private withdrawal`,
    `  4. ${highlight("flow ragequit")}  Saved-workflow public recovery if the easy path is declined`,
    `  5. ${highlight("pools")}          Manual path: browse available pools`,
    `  6. ${highlight("deposit")}        Manual path: deposit into a pool (vetting fee shown before confirming)`,
    `  7. ${highlight("accounts")}       Manual path: check deposit approval status and balances`,
    `  8. ${highlight("withdraw")}       Manual path: withdraw privately (once approved; fee shown before confirming)`,
    `  9. ${highlight("history")}        View transaction history`,
    ...
    `  If the website account predates the entropy upgrade, the CLI may require website migration or website-based recovery before legacy Pool Accounts can be restored safely.`,
    "",
