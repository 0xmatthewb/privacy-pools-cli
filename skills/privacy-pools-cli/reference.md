...
`warnings` may appear on `flow start`, `flow watch`, and `flow status` when the saved workflow has privacy advisories. `flow ragequit` omits it.
...
```json
{
  ...
  "schemas": {
    ...
    "poolAccountStatus": { "values": ["approved", "pending", "poa_required", "declined", "unknown", "spent", "exited"] },
    "errorCategories": {
...
`committedValue` is the net amount after the ASP vetting fee (may be `null`). `label` may be `null`. `nextActions` provides the canonical polling command for agents; follow it to check approval status. All token amounts and block numbers are strings.
...
```json
{
  ...
  "nextActions": [{ "command": "ragequit", "reason": "...", "when": "after_dry_run", "cliCommand": "privacy-pools ragequit ETH --agent --chain mainnet --pool-account PA-1", "args": ["ETH"], "options": { "agent": true, "chain": "mainnet", "poolAccount": "PA-1" } }]
}
```
